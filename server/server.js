const { ipcMain, app, nativeImage, BrowserWindow, Tray, Menu, MenuItem, nativeTheme, screen } = require('electron')
const { EventBus } = require("../js/eventbus.js")
import { CommandLine } from './commandline.js';

import { Util } from '../js/util.js';

const os = require('os');
const path = require('path')
const https = require('https');
const fs = require('fs');
const appIcon = nativeImage.createFromDataURL(`data:image/png;base64,`);
export class Server {
    constructor() {
    }

    get isWindows() {
        return os.platform() == "win32";
    }
    get isMac() {
        return os.platform() == "darwin";
    }
    get isLinux() {
        return !this.isWindows && !this.isMac;
    }
    get systemTypeString() {
        if (this.isWindows) return "windows";
        if (this.isMac) return "mac";
        if (this.isLinux) return "linux";
    }
    get execFolder() {
        return this.getCurrentAppPath();
    }
    async createWindow() {
        const args = {
            icon: appIcon,
            webPreferences: {
                contextIsolation: true,
                enableRemoteModule: false,
                preload: path.join(__dirname, '../preload.js')
            },
            bounds: { width: 800, height: 800 }
        }
        const mainWindow = new BrowserWindow(args);
        mainWindow.setMenuBarVisibility(false);

        mainWindow.loadFile('index.html')

        // Open the DevTools.
        // mainWindow.webContents.toggleDevTools()
        this.window = mainWindow;
        return mainWindow;
    }
    async load() {
        await this.createWindow();
        EventBus.register(this);
        ipcMain.on("eventbus", async (event, { data, className }) => {
            // console.log("Sending to eventbus from web page",data,className)
            await EventBus.post(data, className);
        });
        return this.window;
        // const path = require('path');
    }
    async sendToPageEventBus(object) {
        await this.window.webContents.send('eventbus', { data: object, className: object.constructor.name });
    }
    onRequestTest() {
        console.log("Request test");
        this.sendToPageEventBus(new ResponseTest());
    }
    async onRequestInstallTaskerSettings() {
        console.log("Request install Tasker Settings");
        const userDataPath = app.getPath('userData');
        const filename = "TaskerSettings.apk";
        const downloadPath = path.join(userDataPath, filename);
        const latestInfo = await this.downloadJson("https://api.github.com/repos/joaomgcd/TaskerSettings/releases/latest");
        this.logConsole("Latest info", latestInfo);
        const latestUrl = latestInfo.assets[0].browser_download_url;
        this.logConsole("Latest release", latestUrl);
        await this.downloadFile(latestUrl, downloadPath);
        const androidVersion = await this.getAndroidVersion();
        console.log("Android version", androidVersion);
        const installCommand = `install ${androidVersion >= 34 ? "--bypass-low-target-sdk-block " : ""}${downloadPath}`;
        await this.runAdbCommand(installCommand);
    }
    async getAndroidVersion() {
        return parseInt((await this.runAdbCommand("shell getprop ro.build.version.sdk", true)).out);
    }
    async runAdbCommand(command, serverOnly = false) {
        return await this.onRequestRunCommandLineCommand({
            command: "bin/adb.exe " + command,
            prependCurrentPath: true,
            serverOnly: serverOnly
        })
    }
    getCurrentAppPath() {
        return app.getAppPath().replaceAll("\\", "/");
    }
    async logConsole(log, args) {
        console.log(log, args);

        // this.sendToPageEventBus(new RequestConsoleLog(log,args));
    }
    async onRequestRunCommandLineCommand({ command, args, prependCurrentPath, serverOnly }) {
        if (!command) {
            this.logConsole("Won't run empty command line command");
            return;
        }
        command = command.replace("bin/", `bin/${this.systemTypeString}/`);
        if (command.includes("adb.exe") && (this.isMac || this.isLinux)) {
            command = command.replace(".exe", "");
            const chmod = `chmod 755 ${this.execFolder}/bin/${this.systemTypeString}/adb`;
            this.logConsole("Running chmod", chmod);
            const resultChmod = await CommandLine.run(chmod);
            this.logConsole("chmod result", resultChmod);
        }
        if (prependCurrentPath) {
            // command = (await this.getCurrentAppPath()) + "/" + command;
            command = this.execFolder + "/" + command;
        }
        command = command.replaceAll("resources/app.asar/", "").replaceAll("Resources/app.asar/", "");
        this.logConsole("Running command line Server", command, args);
        try {
            const result = await CommandLine.run(command, args);
            this.logConsole("Command line result", result);
            if (!serverOnly) {
                await this.sendToPageEventBus(new ResponseRunCommandLineCommand(result));
            }
            return result;
        } catch (error) {
            this.logConsole("Command line error", error);
            error = JSON.stringify(error);
            error = JSON.parse(error);
            if (!serverOnly) {
                await this.sendToPageEventBus(new ResponseRunCommandLineCommand(error));
            }
            return error;
        }
    }
    async onRequestToggleDevOptions() {
        this.window.webContents.toggleDevTools()
    }
    async downloadFile(url, downloadPath) {
        // Get the user's app data directory
        const file = fs.createWriteStream(downloadPath);

        return new Promise((resolve, reject) => {
            function handleResponse(response) {
                if (response.statusCode === 200) {
                    response.pipe(file);

                    file.on('finish', () => {
                        console.log("Finished downloading " + url + " to " + downloadPath);
                        file.close(resolve);
                    });
                } else if (response.statusCode === 302 || response.statusCode === 301) {
                    const redirectedUrl = response.headers.location;
                    if (!redirectedUrl) {
                        reject(new Error(`Redirection header missing in response for '${url}'`));
                        return;
                    }
                    // Follow the redirection
                    console.log(`Redirected to ${redirectedUrl}`);
                    https.get(redirectedUrl, handleResponse).on('error', (err) => {
                        fs.unlink(downloadPath, () => { });
                        reject(err);
                    });
                } else {
                    reject(new Error(`Failed to get '${url}' (${response.statusCode})`));
                }
            }

            console.log("Started downloading " + url);
            https.get(url, handleResponse).on('error', (err) => {
                fs.unlink(downloadPath, () => { });
                reject(err);
            });
        });
    }
    async downloadJson(url) {
        return new Promise((resolve, reject) => {
            const options = {
                headers: {
                    'User-Agent': 'com.joaomgcd.taskerpermissions'
                }
            };

            https.get(url, options, (response) => {
                let data = '';

                if (response.statusCode === 200) {
                    response.on('data', (chunk) => {
                        data += chunk;
                    });

                    response.on('end', () => {
                        try {
                            const json = JSON.parse(data);
                            console.log('Finished downloading and parsing JSON from ' + url);
                            resolve(json);
                        } catch (error) {
                            reject(new Error(`Failed to parse JSON from '${url}': ${error.message}`));
                        }
                    });
                } else if (response.statusCode === 302 || response.statusCode === 301) {
                    const redirectedUrl = response.headers.location;
                    if (!redirectedUrl) {
                        reject(new Error(`Redirection header missing in response for '${url}'`));
                        return;
                    }
                    // Follow the redirection
                    console.log(`Redirected to ${redirectedUrl}`);
                    downloadJson(redirectedUrl).then(resolve).catch(reject);
                } else {
                    reject(new Error(`Failed to get '${url}' (${response.statusCode})`));
                }
            }).on('error', (err) => {
                reject(err);
            });
        });
    }
}

class ResponseRunCommandLineCommand {
    constructor(result) {
        Object.assign(this, result);
    }
}

class ResponseTest { }
class RequestConsoleLog {
    constructor(log, args) {
        this.log = log;
        this.args = args;
    }
}